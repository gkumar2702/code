...
df_ads['MAJ_OTG_ID']=df_ads['MAJ_OTG_ID'].astype(np.int64)
...
def count_outage(group):
  ...
  df_temp = df_ads[['OUTAGE_ID','CREATION_DATETIME']]
  df_temp['minutes'] = (group['CREATION_DATETIME'][0] - df_temp['CREATION_DATETIME']).dt.total_seconds().div(60)
...
df_ads = df_ads.groupby(['OUTAGE_ID'], as_index = False).apply(count_outage)
...
df_ads.rename({'MAJ_OTG_ID':'INSERTION_TIME'},axis=1,inplace=True)
#df_ads.drop(['MAJ_OTG_ID'],axis=1,inplace=True)
